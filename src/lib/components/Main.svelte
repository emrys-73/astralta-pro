<script>
    import CallToAction from "./Astralta/Landing_CTA.svelte"
    import Subtitle from "./Astralta/Subtitle.svelte";

</script>

    <div class="hero min-h-screen" style="background-image: url(/astralta/bg/mesh_bg.JPG);">
        <div class="bg-gradient-to-b from-black to-transparent w-full h-3/4 absolute top-0 opacity-0">
            
        </div>
        <div class="hero-overlay bg-opacity-0"></div>
            <div class="text-center text-neutral-content">
            <div class="max-w-2xl">
                <div class="relative">
                    <h1 class="mb-0 text-7xl font-bold text-white drop-shadow-lg">Your own personalised AI</h1>
                    
                
                    <!-- <p class="mb-5  text-white font-light drop-shadow-lg">tailored AI assistant for your business</p>  -->
                    <div class=" align-middle text-center">
                        <Subtitle/>
                    </div>
                    
                    
                    <CallToAction/>
                    
                </div>
                
            </div>
        </div>
        
      </div>
